...
QScriptValue include(QScriptContext *context, QScriptEngine *engine) {
	...
	const auto &scriptValue =
	    engine->globalObject().property(scriptEngineWorkerName);
	if (auto scriptWorkerValue =
		qobject_cast<ScriptEngineWorker *>(scriptValue.toQObject())) {
		auto connection = (QThread::currentThread() != engine->thread())
				      ? Qt::BlockingQueuedConnection
				      : Qt::DirectConnection;
		QMetaObject::invokeMethod(
		    scriptWorkerValue,
		    [scriptWorkerValue, filename, engine]() {
			    scriptWorkerValue->evalInclude(filename, engine);
		    },
		    connection);
	}
...
QScriptValue print(QScriptContext *context, QScriptEngine *engine) {
	...
	for (int i = 0; i < argumentCount; ++i) {
		std::function<QString(const QVariant &)> prettyPrinter =
		    [&prettyPrinter](QVariant const &elem) {
			    auto const &arrayPrettyPrinter =
				[&prettyPrinter](const QVariantList &array) {
					...
					res.reserve(100000);
					res.append(
					    "[" % prettyPrinter(array.first()));

					for (auto i = 1; i < arrayLength; ++i) {
						res.append(
						    ", " %
						    prettyPrinter(array.at(i)));
					}
	...
	if (auto script = qobject_cast<TrikScriptControlInterface *>(
		scriptValue.toQObject())) {
		result.append('\n');
		QTimer::singleShot(0, script, [script, result]() {
			Q_EMIT script->textInStdOut(result);
		});
		/// In case of user loop with `print' this gives some time for
...
ScriptEngineWorker::ScriptEngineWorker(
	...
	connect(mScriptControl, &TrikScriptControlInterface::quitSignal, this,
		&ScriptEngineWorker::onScriptRequestingToQuit);
	connect(this, &ScriptEngineWorker::getVariables, &mThreading,
		&Threading::getVariables);
	connect(&mThreading, &Threading::variablesReady, this,
		&ScriptEngineWorker::variablesReady);

	registerUserFunction("print", print);
	registerUserFunction("include", include);
}

void ScriptEngineWorker::brickBeep() { mBrick->playTone(2500, 20); }

void ScriptEngineWorker::evalInclude(const QString &filename,
...
void ScriptEngineWorker::stopScript() {
	...
	while (mState == starting) {
		// Some script is starting right now, so we are in inconsistent
		// state. Let it start, then stop it.
		locker.unlock();
	...
	if (mMailbox) {
		...
		/// @todo: here script will continue to execute and may execute
		/// some statements before it will eventually be stopped by
		/// mThreading.reset(). But if we do mThreading.reset() before
		/// mMailbox->stopWaiting(), we will get deadlock, since
		/// mMailbox->stopWaiting() shall be executed in already stopped
		/// thread. Actually we shall stop script engines here, do
		/// mMailbox->stopWaiting(), then stop threads.
	}

	QMetaObject::invokeMethod(&mThreading, &Threading::reset,
				  Qt::QueuedConnection);
	mThreading.reset();
	...
	if (mDirectScriptsEngine) {
		...
		const auto &msg =
		    mDirectScriptsEngine->hasUncaughtException()
		...
		QTimer::singleShot(0, this,
				   [this]() { mDirectScriptsEngine.reset(); });
...
void ScriptEngineWorker::runDirect(const QString &command, int scriptId) {
	...
	QMetaObject::invokeMethod(this,
				  std::bind(&ScriptEngineWorker::doRunDirect,
					    this, command, scriptId));
}
...
void ScriptEngineWorker::doRunDirect(const QString &command, int scriptId) {
	...
	if (mDirectScriptsEngine) {
		...
		/// If script was stopped by quit(), engine will already be
		/// reset to nullptr in ScriptEngineWorker::stopScript.
		QString msg;
		if (mDirectScriptsEngine &&
		    mDirectScriptsEngine->hasUncaughtException()) {
			QLOG_INFO() << "ScriptEngineWorker : ending "
				       "interpretation of direct script";
			msg = mDirectScriptsEngine->uncaughtException()
				  .toString();
...
void ScriptEngineWorker::startScriptEvaluation(int scriptId) {
	QLOG_INFO() << "ScriptEngineWorker: starting script" << scriptId
...
void ScriptEngineWorker::evalExternalFile(const QString &filepath,
					  QScriptEngine *const engine) {
	if (QFileInfo::exists(filepath)) {
		engine->evaluate(trikKernel::FileUtils::readFromFile(filepath),
				 filepath);
		if (engine->hasUncaughtException()) {
			...
			const auto &error =
			    tr("Line %1: %2")
				.arg(QString::number(line), message) +
			    "\nBacktrace" + backtrace;
			emit completed(error, mScriptId);
			QLOG_ERROR()
			    << "Uncaught exception with error" << error;
		}
	} else {
		emit completed(tr("File %1 not found").arg(filepath),
			       mScriptId);
...
void ScriptEngineWorker::onScriptRequestingToQuit() {
	if (!mScriptControl->isInEventDrivenMode()) {
		// Somebody erroneously called script.quit() before entering
		// event loop, so we must force event loop for script and only
		// then quit, to send completed() signal properly.
		mScriptControl->run();
	}
...
QScriptEngine *ScriptEngineWorker::createScriptEngine(bool supportThreads) {
	...
	QLOG_INFO() << "New script engine" << engine
		    << ", thread:" << QThread::currentThread();
	...
	qScriptRegisterMetaType(engine, &timeValToScriptValue,
				&timeValFromScriptValue);
	...
	engine->globalObject().setProperty("script",
					   engine->newQObject(mScriptControl));
	engine->globalObject().setProperty(scriptEngineWorkerName,
					   engine->newQObject(this));

	if (mMailbox) {
		engine->globalObject().setProperty(
		    "mailbox", engine->newQObject(mMailbox));
	}

	// Gamepad can still be accessed from script as brick.gamepad(),
	// 'gamepad' variable is here for backwards compatibility.
	if (auto gamepad = mBrick->gamepad()) {
		engine->globalObject().setProperty("gamepad",
						   engine->newQObject(gamepad));
	}

	if (supportThreads) {
		engine->globalObject().setProperty(
		    "Threading", engine->newQObject(&mThreading));
	}
...
QScriptEngine *
ScriptEngineWorker::copyScriptEngine(const QScriptEngine *const original) {
	QScriptEngine *const result = createScriptEngine();

	QScriptValue globalObject = result->globalObject();
	Utils::copyRecursivelyTo(original->globalObject(), globalObject,
				 result);
	result->setGlobalObject(globalObject);

	// We need to re-eval system.js after global object copying because
	// functions did not get copied by copyRecursivelyTo, and existing ones
	// were overwritten by copying.
	evalSystemJs(result);

	return result;
}

void ScriptEngineWorker::registerUserFunction(
    const QString &name, QScriptEngine::FunctionSignature function) {
	mRegisteredUserFunctions[name] = function;
}

void ScriptEngineWorker::addCustomEngineInitStep(
...
void ScriptEngineWorker::evalSystemJs(QScriptEngine *const engine) {
	const QString systemJsPath =
	    trikKernel::Paths::systemScriptsPath() + "system.js";
	evalExternalFile(systemJsPath, engine);

	for (const auto &functionName : mRegisteredUserFunctions.keys()) {
		QScriptValue functionValue =
		    engine->newFunction(mRegisteredUserFunctions[functionName]);
...
QStringList ScriptEngineWorker::knownMethodNames() const {
	...
	TrikScriptRunnerInterface::Helper::collectMethodNames(
	    result, &trikControl::BrickInterface::staticMetaObject);
	TrikScriptRunnerInterface::Helper::collectMethodNames(
	    result, mScriptControl->metaObject());
	if (mMailbox) {
		result.insert("mailbox");
		TrikScriptRunnerInterface::Helper::collectMethodNames(
		    result, mMailbox->metaObject());
	}
	TrikScriptRunnerInterface::Helper::collectMethodNames(
	    result, mThreading.metaObject());
