...
/// Widget that is used to configure settings of robot group communication via
/// mailboxes.
...
public:
	/// Constructor.
	/// @param mailbox - reference to mailbox object which we will configure
	/// here.
...
public:
	/// Called when 'Connect' button is pressed, initiates connection to
	/// leader robot.
	Q_INVOKABLE void onConnectButtonClicked(QString newThirdFourthIpPart);

	/// Called when hull number of this robot is changed. Hull number is
	/// stored in local settings, but used only after reconnect.
	Q_INVOKABLE void onHullNumberChanged(int newHullNumber);
