...
AccelerometerPublisher::AccelerometerPublisher(
	...
	: topic_(topic)
	, device_(brick->accelerometer())
{
	if (!device_) {
		RCLCPP_ERROR(node->get_logger(),
			"AccelerometerPublisher: accelerometer not available in Brick");
		throw std::runtime_error("AccelerometerPublisher: no accelerometer");
	}

	RCLCPP_INFO(node->get_logger(),
		"AccelerometerPublisher: -> topic '%s'", topic.c_str());

	publisher_ = node->create_publisher<geometry_msgs::msg::Vector3Stamped>(topic, 10);
	clock_     = node->get_clock();
}

void AccelerometerPublisher::publish()
{
